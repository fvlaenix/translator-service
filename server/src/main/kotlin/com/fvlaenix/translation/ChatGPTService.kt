...
class ChatGPTService(
  private val translator: Translator = TextModelServiceTranslator(OpenAIAPIServiceImpl()),
) : TranslationServiceGrpcKt.TranslationServiceCoroutineImplBase() {

  constructor(model: String) : this(TextModelServiceTranslator(OpenAIAPIServiceImpl(model)))
    ...
    return withLog("translation", "text:${request.text.take(256)}") {
      ...
      translationResponse {
        runCatching {
          ...
          this@translationResponse.text =
            result.joinToString(separator = "\n") { it.translation ?: throw Exception("Translation failed") }
        }
          .getOrElse {
            LOG.log(Level.SEVERE, "Exception while trying to get translation", it)
            this.error =
              "Exception while trying to get translation. You can tag the person who ran the bot to see what the problem might be"
          }
      }
    }
  }
  ...
  override suspend fun translationFile(request: TranslationFilesRequest): TranslationFilesResponse {
    return withLog("translationFile", "filenames:${request.requestsList.joinToString { it.fileName }}") {
      ...
      val original = files.flatMap { file ->
        file.blocksList.map { block ->
          TextTranslation(
            block.text,
            if (block.hasTranslation()) block.translation else null
          )
        }
      }
      ...
      if (translation.size != original.size) {
        ...
        return@withLog translationFilesResponse {
          this.error =
            "Internal error: Sizes of translation and original is not matched. Report this to person who run bot"
        }
      }
      var it = 0
